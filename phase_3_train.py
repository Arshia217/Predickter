...
############################### Solution Using XG Boost for score calculation and Logistic Regression for Win Probability ###############################
import os
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
...
# 🚀 Injected Advanced Non-Linear Engine
from xgboost import XGBRegressor

def train_and_evaluate():
    MATRIX_PATH = os.path.join("data", "processed", "modeling_matrix.csv")
    
    print("🔄 Loading upgraded Phase 2 matrix...")
    if not os.path.exists(MATRIX_PATH):
        raise FileNotFoundError(f"Missing modeling matrix at {MATRIX_PATH}.")
    ...
    feature_cols = [
        ...
        'stadium_avg_runs', 
        'stadium_volatility', 
        'stadium_boundary_density', 
        'index_differential'
    ]
    
    X = df[feature_cols]
    y_reg = df['innings1_runs']
    y_clf = df['team1_won']
    ...
    # =========================================================================
    # TASK 1: XGBoost Regressor (Predicting Innings 1 Runs Magnitude)
    # =========================================================================
    print("\n🌲 Training Non-Linear XGBoost Regressor Model...")
    # Fine-tuned with conservative hyperparameters to prevent overfitting on sports data
    ...
    print("--- 📉 XGBOOST REGRESSOR METRICS (Score Prediction) ---")
    print(f"Mean Absolute Error (MAE) : {mean_absolute_error(y_reg_test, y_reg_pred):.2f} runs")
    print(f"Root Mean Squared Error (RMSE): {np.sqrt(mean_squared_error(y_reg_test, y_reg_pred)):.2f} runs")
    print(f"R-squared (R2) Score          : {r2_score(y_reg_test, y_reg_pred):.4f}")
    
    print("\n💡 Feature Importances (XGBoost Score Prediction):")
    for feat, importance in zip(feature_cols, reg_model.feature_importances_):
